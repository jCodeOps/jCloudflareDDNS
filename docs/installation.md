...
After extracting the generated `target/jcloudflareddns-0.1.0-RC1-distribution.tar.gz`, run `bin/jcloudflareddns --help` from the extracted directory. The launcher uses `JAVA_HOME` when set, otherwise it resolves `java` from `PATH`.
